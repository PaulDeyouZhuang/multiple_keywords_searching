...
#將資料夾home/secControl/內的檔案listOfKeywords_html.xlsx複製到目前位置
cp ~/controlFolder/listOfKeywords_html.xlsx .
...
#否則
	...
	#那麼
	then
		#就把變數$string_forGrep設為此keyword加上\w*\b
		#\w*\b是為了之後的搜索，只搜索一個單字(單字後端可以加任何a-z的字母，
		#遇到非a-z的字母就停止)
		string_forGrep="$x\w*\b"

		#變數str_folderName_tmp設為". "+"keyword的第一個單字為"+"_html_"(為了命名資料夾)
		str_folderName_tmp=". $x$Ofhtml"
...
#例如5. Period_html_2018-01-24-11:21:43
...
#建一個資料夾，資料夾名字叫做html_files_indexed
mkdir html_files_indexed
#回到上上層資料夾(all_html/)
cd ../..

#在資料夾(all_html)中，針對所有的.htm檔，去搜索符合目前一組keywords的字附近的3行前後文
#將搜索的結果(含檔案原始名稱)放到資料夾$str_folderName內的檔案grep_f_c.out
grep -i -w -C 1 "$string_forGrep" *.htm > $str_folderName/grep_f_c.out
#將資料夾$str_folderName內的檔案grep_f_c.out由windows的dos換行格式轉換為unix系統的換行格式
dos2unix  $str_folderName/grep_f_c.out
#把grep_f_c.out內的第一欄檔名的.htm拿掉，產生新的檔案filenames_notyped_delimit_dup_multiple.out
awk -F '.htm' '{print $1}' $str_folderName/grep_f_c.out > $str_folderName/filenames_notyped_delimit_dup_multiple.out
#在資料夾(all_html)中，針對所有的.htm檔，去搜索符合目前一組keywords的字附近的3行前後文
#將搜索的結果(不含檔案原始名稱)放到資料夾$str_folderName內的檔案context_delimit_multiple.out
grep -i -w -h -C 1 "$string_forGrep" *.htm > $str_folderName/context_delimit_multiple.out 
...
#貼到新的檔案twoColumns_notHyped_delimit_multiple，兩欄以tab相隔
paste -d '\t' filenames_notyped_delimit_dup_multiple.out context_delimit_multiple.out > twoColumns_notHyped_delimit_multiple

#將資料夾home/scripts_html/內的檔案add_hyperLink_index.pl複製至此資料夾
cp ~/scripts_html/add_hyperLink_index.pl .
...
#詳細過程請參閱資料夾home/scripts_html/內的檔案add_hyperLink_index.pl
./add_hyperLink_index.pl "${arrOfIndex[@]}" > twoColumns_Hyped_delimit_multiple

#將資料夾home/scripts_html/內的檔案fromMultipleFileNames_to_one.pl複製至此資料夾
cp ~/scripts_html/fromMultipleFileNames_to_one.pl .
...
#詳細過程請參閱資料夾home/scripts_html/內的檔案fromMultipleFileNames_to_one.pl
...
#將資料夾home/scripts_html/內的檔案stripOffNewLine.pl複製至此資料夾
cp ~/scripts_html/stripOffNewLine.pl .
#執行檔案stripOffNewLine.pl
#檔案stripOffNewLine.pl的目的是為了
#要將檔案context_toBeStripped每一區塊的多行前後文字
#化為只剩一行，並放到新的檔案context_stripped中
#詳細過程請參閱資料夾home/scripts_html/內的檔案stripOffNewLine.pl
./stripOffNewLine.pl

#將檔案filenames_notyped_delimit_dup_single.out(一欄) 和
#檔案context_stripped(另一欄)，
#貼到新的檔案two_columns_one_inEachSeparator1，兩欄以tab相隔
paste -d '\t' filenames_notyped_delimit_dup_single.out context_stripped > two_columns_one_inEachSeparator1

#將資料夾home/scripts_html/內的檔案clearTags.pl複製至此資料夾
cp ~/scripts_html/clearTags.pl .
#執行檔案clearTags.pl
#檔案clearTags.pl的目的是為了
#要將檔案two_columns_one_inEachSeparator1內，尚有不完全的html tags，
#或原始檔案中被註解過的html tags移除
#詳細過程請參閱資料夾home/scripts_html/內的檔案clearTags.pl
./clearTags.pl


#將資料夾home/scripts_html/內的檔案remove_delimiters.pl複製至此資料夾
cp ~/scripts_html/remove_delimiters.pl .
...
#詳細過程請參閱資料夾home/scripts_html/內的檔案remove_delimiters.pl
./remove_delimiters.pl

#將檔案two_columns_no_separator的第一個欄位(檔名)取出，檔名後方加上檔案類型(.htm)後，
#放到檔案filenames_toBeAppended中
awk -F '\t' '{print $1".htm"}' two_columns_no_separator > filenames_toBeAppended


#將資料夾home/scripts_html/內的檔案SECinfo_Index_File_no_duplicate_withWC.csv複製至此資料夾
cp ~/scripts_html/SECinfo_Index_File_no_duplicate_withWC.csv .
#將資料夾home/scripts_html/內的檔案append.sh複製至此資料夾
cp ~/scripts_html/append.sh .
#執行檔案append.sh
...
#詳細過程請參閱資料夾home/scripts_html/內的檔案append.sh
...
#將資料夾home/scripts_html/內的檔案addHTML_headers.pl複製至此資料夾
cp ~/scripts_html/addHTML_headers.pl .
#執行檔案addHTML_headers.pl
#檔案addHTML_headers.pl的目的是加html的headers到pre_menu.html中，
#出來的結果放到新的檔案menu.html
#詳細過程請參閱資料夾home/scripts_html/內的檔案addHTML_headers.pl
./addHTML_headers.pl
...
#將資料夾home/scripts_html/內的檔案cp_desired_html.sh複製至此資料夾
cp ~/scripts_html/cp_desired_html.sh .
#執行檔案cp_desired_html.sh
#檔案cp_desired_html.sh的目的是為了將檔案filenames_notyped_nodelimit_nodup_single.out中，
#有出現過的檔名的檔案，複製到資料夾tmp_html/中
#詳細過程請參閱資料夾home/scripts_html/內的檔案cp_desired_html.sh
./cp_desired_html.sh
...
#將資料夾home/scripts_html/內的檔案add_keyIndexToAllFiles.pl複製至此資料夾
cp ~/scripts_html/add_keyIndexToAllFiles.pl .
...
#詳細過程請參閱資料夾home/scripts_html/內的檔案add_keyIndexToAllFiles.pl
...
#將資料夾home/scripts_html/內的檔案index.html複製至此資料夾html/html_files_indexed中
cp ~/scripts_html/index.html html/html_files_indexed
...
#將檔案$str_folderName.zip移動到home/resultFolder/中
